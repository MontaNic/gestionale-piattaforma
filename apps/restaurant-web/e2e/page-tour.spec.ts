...
import { RESTAURANT_MANIFEST, type DynamicStrategy } from './page-manifest';

/**
 * page-tour.spec.ts — smoke funzionale per-ruolo (restaurant-web).
 *
 ...
 *     (NON esiste un error.tsx applicativo → marker reale; TD-no-error-boundary);
 *  4. parte una richiesta MUTANTE (POST≠login / PUT / PATCH / DELETE).
 *
 * READ-ONLY ATTIVO (invariante 1): il guard fa `route.abort()` su ogni metodo
 * mutante → nessuna mutazione raggiunge il DB prod condiviso.
 *
 * Target: URL pubblico via PLAYWRIGHT_BASE_URL (https://food.studiodesk.cloud).
...
async function settle(page: Page): Promise<void> {
  // Shell pronta = AuthGate ha caricato /me (restaurant ha solo la superficie operatore).
  await page
...
for (const role of RESTAURANT_MANIFEST) {
