...
// ─────────────────────────────────────────────────────────────────────────────
// 1-2. Catalogo RBAC (permessi + template di ruolo) — `prisma/rbac-catalog.ts`
// ─────────────────────────────────────────────────────────────────────────────
// Estratto da qui perché il gate di deriva (`scripts/check-role-permissions-
// drift.ts`) deve leggerne il set atteso senza eseguire il seed: questo file ha
// un side-effect top-level in fondo (`withSystemContext(() => main())`).
// Il catalogo resta la fonte di verità unica per entrambi.
// ─────────────────────────────────────────────────────────────────────────────
import { PERMISSIONS, ROLE_TEMPLATES } from './rbac-catalog';
