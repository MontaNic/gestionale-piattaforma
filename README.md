...
Comandi di sviluppo (workspace-wide via Turborepo, attivi quando esisteranno le app):

```bash
pnpm dev          # avvia tutte le app in modalità dev
pnpm build        # build di tutti i workspace
pnpm lint         # ESLint su tutti i workspace
pnpm typecheck    # tsc --noEmit su tutti i workspace
pnpm test         # test runner di ciascun workspace
...
- **Branch**: `main` (stabile), `feature/*`, `fix/*` (vedi C12 brief).
