...
9. ~~**Miglioramento pre-push hook** — parsing stdin formato git pre-push per distinguere push regolari da delete.~~ **RISOLTO 2026-07-30 (#199)** come effetto del fix di `TD-prepush-hook-blocca-tag` — vedi la voce nella sezione "Qualità codice / processo".
...
- [x] ~~**Miglioramento pre-push hook**: parsing stdin formato git pre-push per distinguere push regolari da delete (`local-sha == 0000...` indica delete). Elimina la necessità di `--no-verify` per cancellazioni remote legittime di branch diverso da `main` quando ci si trova su `main`.~~ — **RISOLTO 2026-07-30 (#199)**, come effetto del fix di `TD-prepush-hook-blocca-tag`. L'hook ora legge stdin e blocca **se e solo se** una riga ha `refs/heads/main` come remote ref: qualunque push che non tocchi `main` passa anche stando su `main`, delete inclusi, senza `--no-verify`. Provato dai casi 5 e 7 della matrice ([ADR-0004, amendment 2026-07-30](./docs/architecture/ADR-0004-local-git-hooks.md#prova-di-efficacia)). Nota: il delete **di `main`** va nella direzione opposta — prima passava, ora è bloccato (caso 3).
...
**Cosa esisteva prima** (STOP 0 read-only): due `tailwind.config.ts` **byte-identiche** (`TD-tailwind-config-dup`) e due `globals.css` che divergevano per 3 variabili — senza che nulla nel codice dicesse quali fossero divergenza **voluta** e quali copia da riallineare a mano. ⚠️ **Correzione al ground truth di partenza**: il blocco `.dark` era dato per "identico nei due globals". **Non lo era**: `--primary`, `--primary-foreground` e `--ring` divergono **in entrambi i temi** → seam preesistente = 3 variabili × 2 temi. Il numero corretto viene dal diff contro il build reale, non dall'ispezione a vista. Dopo questa PR il seam è **7 variabili × 2 temi** (le 3 + `--accent-soft`/`-foreground` e `--brand`/`-foreground`).

**Struttura**: `packages/ui/src/tokens.css` (base condivisa: neutri, superfici, tipografia, radii, regole base `*`/`body`) + `packages/ui/tailwind-preset.ts` (mapping nome-Tailwind → token, una volta sola). Le app tengono la propria `tailwind.config.ts` solo per `content` e `plugins`, e nei `globals.css` resta il **blocco seam** per-verticale: **7 variabili × 2 temi** (`--primary`/`-foreground`, `--ring`, `--accent-soft`/`-foreground`, `--brand`/`-foreground`). Token additivi non adottati (`--font-display`, `--warn`, `--warn-soft`) → zero impatto sul rendering.
...
**TD**: 🆕 `TD-visual-regression-net` (baseline Playwright su `page-manifest` × light/dark — **trigger:** prima di P4, il ritocco delle primitive condivise; nasce come **strumento di iterazione, NON gate**, dato lo stato pre-lancio senza clienti reali). ⚠️ **Il debito è già coperto a metà**: il diff dei valori computati usato qui è il cugino economico della rete screenshot per ogni **cambio di token** — deterministico, nessuna baseline binaria, nessun flake. Va riusato prima di P4, e la rete screenshot va costruita solo per il residuo che non vede (geometria, spaziatura, ritorni a capo — cioè proprio ciò che P4 tocca). 🆕 `TD-smoke-punta-solo-a-prod` (`page-tour.spec.ts` ha `baseURL` sull'URL pubblico in entrambe le app → **non può validare un branch pre-merge**: eseguirlo su una PR misura la prod, non la PR. Limite dell'**infrastruttura di test**, non di questa PR; qui sostituito dalla verifica runtime su stack dev buildato dal branch. **Trigger:** quando serve una smoke per-ruolo gatante in CI su codice non deployato). ✅ `TD-tailwind-config-dup` risolto.
