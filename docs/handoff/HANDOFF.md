...
Note Spese (**PR-1 schema ✅ · PR-2 CRUD/storage ✅ · PR-3 state machine ✅ · PR-3a allegati read ✅ · PR-4 UI operatore ✅ 2026-07-24**, [ADR-0077](../architecture/ADR-0077-note-spese-pr4-ui-operatore.md); backend completo (18 test) + FE operatore provato a runtime su dev con ruolo non-superuser. **PR-5 = pannello approvazione** (`notespese.approva`: vista separata, approva/respingi con motivo) — unico residuo del blocco), varianti, accorpamento, sconti, allergeni, cassa, kiosk (PR-3b).
