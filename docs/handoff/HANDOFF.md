...
Sul chiuso più recente, tre cose che non si deducono dal diff:
...
- **Il fix chiude un buco che nessuno aveva notato**: il delete di `main` (`git push origin :main`) **passava**, perché si esegue da un'altra branch e l'hook guardava solo `HEAD`. Ora è coperto.
- **Ampliamento voluto** (D3): cade anche il falso positivo "sono su `main` e pusho un feature branch". Non tocca `refs/heads/main`, quindi non poteva inquinarne la history.

Efficacia provata su una matrice di 9 casi in repo usa-e-getta, con lo stesso runner `sh -e` della produzione e il file reale sotto test — dettaglio nell'amendment. La pubblicazione dei 4 tag `deploy/*` è il passo successivo e **non è ancora avvenuta**: si fa dopo il merge, ed è la verifica finale del debito (`git ls-remote --tags origin` non più vuoto).
