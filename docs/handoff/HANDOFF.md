...
> **Snapshot:** Main @ 57c9145 (+1 commit docs(handoff) in arrivo via PR).
> **Data:** 2026-06-10.
...
Catalogo permessi: **35** (`preventivi.{visualizza,gestisci}` da STOP-e1; dashboard riusa `anagrafica.cliente.visualizza`, nessun permesso nuovo).
...
Moduli di dominio StudioDesk ancora mancanti nel TS: Comunicazioni, Documenti & Circolari, Scadenze (calendario fiscale), Knowledge Base, Questionari, Agevolazioni, Team. La card-grid della dashboard (ADR-0038) è predisposta ad accoglierli come card man mano che nascono. **Nessuna card-placeholder per moduli non costruiti** (YAGNI/Pattern 43).
...
- **Altri moduli operatore-studio** (Scadenze/Documenti/Comunicazioni) — verso il completamento del livello 1.
...
- **TD-RLS-dashboard candidate** (nuovo, ADR-0038) — endpoint `/dashboard/stats` non esercitato da `rls-isolation` e2e; isolamento verificato applicativamente (scenario A/B di `dashboard-stats`) + a runtime non-superuser. Bassa priorità: gli aggregati riusano le stesse policy delle CRUD già esercitate.
- **TD candidate — seed utente non-superuser studio-demo** (da ADR-0037) — gating `preventivi.*`/`anagrafica.cliente.*` verificato solo a livello codice, non runtime (`admin@studio.local` è Super Admin, 35 permessi). Fix: seed utente Direzione/Admin sede. ~20min.
...
- e2e accountant: `pnpm --filter @gestionale/accountant-api test:e2e` (Testcontainers, locale). Suite attuale: aziende-crud 11 + referenti-crud 11 + rls-isolation 10 + preventivi-crud 12 + dashboard-stats 4 = **48**.
...
- **Main @ 57c9145** (+1 commit docs(handoff) in arrivo via PR) — `feat(accountant): dashboard KPI operatore-studio (endpoint /dashboard/stats + card-grid) (#85)`
- Working tree pulito, branch unico `main` allineato a `origin/main`. Nessun branch feature pendente.
- PR mergiate nella sessione 2026-06-10: #83 (preventivi UI, ADR-0037), #84 (RLS isolation e2e preventivi, LEAN), #85 (dashboard, ADR-0038).

### ADR

Fino a **ADR-0038**. Ultimi del 2° verticale: 0029/0030 (skeleton), 0031/0032 (aziende BE/UI), 0033/0034 (referenti BE/UI), 0035 (RLS isolation anagrafica), 0036 (preventivi backend), 0037 (preventivi UI), 0038 (dashboard operatore-studio).
...
- Tenant demo: `studio-demo` (`admin@studio.local` / `Studio123!`, Super Admin 35 permessi), + `studio-acme` per test isolamento. Seed demo accountant: 5 aziende (AZ001-005, AZ003 attivo=false), 3 referenti, 2 preventivi (PREV-2025-001/002 su AZ001, aliquote miste 22%/10%).
