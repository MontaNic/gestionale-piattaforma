...
Note Spese (**PR-1 schema+fondamenta ✅ · PR-2 CRUD+allegati/storage ✅ · PR-3 state machine+gating ✅ 2026-07-23**, branch `feat/note-spese-state-machine`, [ADR-0076](../architecture/ADR-0076-note-spese-pr3-state-machine.md); backend **completo** — 15 test. DP-1 `respinta→inviata` diretta e DP-2 azzeramento decisionali prese su §4.3 _ricostruito_ (la spec persistita dice `respinta→bozza`) → reversibili. **PR-4/5 = FE** (§8: calendario/elenco, form con hint, pannello approvazione gated `notespese.approva`)), varianti, accorpamento, sconti, allergeni, cassa, kiosk (PR-3b).
