...
- **TD-bootstrap-verticale** — [tenants.service.ts](../../packages/auth/src/tenants/tenants.service.ts) clona **tutti** gli 11 template `isDefault=true` → un tenant creato via API riceverebbe i ruoli del verticale sbagliato (es. un tenant restaurant otterrebbe Socio/Praticante/Segreteria). Latente: nessun tenant via API esiste. Causa-radice: assenza di una dimensione `verticale` nei dati (oggi la curatela vive solo nel seed imperativo).
