...
## Empirical discoveries (4 in sessione 10 + 3 in PR 2 sessione 12 + 2 in PR cleanup sessione 12)

> Counter ADR-internal. Per il counter **cross-ADR cumulative globale** (oggi 43), vedi [PROGRESS.md](../../PROGRESS.md). Numero discovery ID (`#NN`) è cross-ADR autoritativo — assegnato in ordine cronologico di scoperta tra tutti gli ADR del progetto.
...
### Discovery #39 — `noUncheckedIndexedAccess` strict frontend (semanticamente TD-AJ)

**Macro-task:** PR 2 sessione 12 Fase 2 (TD-AJ frontend mapping table) — moved da ADR-0013 in PR cleanup post-merge sessione 12 (cross-link correction).

**Root cause:** `tsconfig` apps/web ha `noUncheckedIndexedAccess: true` (strict mode). `Record<string, string>` lookup ritorna sempre `string | undefined`, anche su dot-access (`ERROR_CODE_MESSAGES.E_UNKNOWN`). Senza fix, `messageForErrorCode()` rifiuta type-check (TS2322).

**Fix:** in [apps/web/src/lib/error-codes.ts](../../apps/web/src/lib/error-codes.ts) estratto `FALLBACK_MESSAGE` come `const` string literal (non Record lookup). Helper diventa `return ERROR_CODE_MESSAGES[code] ?? FALLBACK_MESSAGE` — coalesce singolo type-safe.

**Lesson generalizzabile:** mapping table i18n-ready in TS strict mode richiedono **fallback letterale separato** dal Record. Pattern da seguire per future estensioni (i18next, locale tables, status badge maps ecc.).

### Discovery #43 — DTO class-validator shape non coerente con taxonomy TD-AJ

**Macro-task:** PR cleanup post-merge sessione 12 (formalizzazione empirica gap UX coverage).

**Root cause:** smoke browser post-merge Nicolò ha rivelato che le response 400 emesse da NestJS `ValidationPipe` (su DTO con `class-validator` decorators) usano shape default `{statusCode: 400, error: 'Bad Request', message: ['email must be an email', ...]}` (campo `message` come **array di stringhe**, non singola stringa). Questa shape:

1. Manca del field `errorCode` (taxonomy [TD-AJ](#td-aj-resolution-pr-2) introdotta in PR 2)
2. Ha `message` array invece di string — incompatibile con `ApiError` frontend (`message: string`)
3. Manca `timestamp` field

Frontend `parseError()` riceve `body.errorCode = undefined` → fallback `E_UNKNOWN` → `messageForErrorCode('E_UNKNOWN')` ritorna "Si è verificato un errore. Riprova." — generica, perde dettaglio validation field-specific.

**Fix applicato in PR 2:** nessuno (out of scope DP3.1 lockato su `/auth/login` 401 only). Scope allargato in TD-AY ADR-0016.

**Lesson generalizzabile:** pattern senior NestJS per taxonomy `errorCode` unificata richiede **custom `ValidationPipe` exception factory** (e.g. `new ValidationPipe({ exceptionFactory: errors => new BadRequestException({statusCode: 400, errorCode: 'E_VALIDATION_FAILED', message: 'Dati non validi', details: errors.map(...), timestamp: new Date().toISOString()}) })`) per allineare 400 validation alla shape `errorCode` taxonomy. Bonus: `details` array dettaglio field-specific per UX form (highlight field error inline).
...
| ~~**TD-AY**~~ (scope espanso post-cleanup PR sessione 12) | ~~Allineamento taxonomy `errorCode` unificata cross-endpoint **COMPLETA**~~ — **RESOLVED sessione 15** ([§TD-AY resolution](#td-ay-resolution-sessione-15)). Decisione A2: `GlobalHttpExceptionFilter` `@Catch(HttpException)` registrato via `useGlobalFilters` normalizza shape `{statusCode, errorCode, message, ...extras}` cross-endpoint (4 detection branches + fallback status-based). LockoutExceptionFilter refactor `extends GlobalHttpExceptionFilter` per DI priority preservation. | ✅ DONE                    | ~2h effettivi (DP1-DP3 + 2 discovery emerse) |
...
## TD-AY resolution (sessione 15)

**Status:** RESOLVED 2026-05-19 via PR `chore/td-ay-td-be-sub2-cleanup` (atomic closure TD-AY + TD-BE Sub-2).

### Decision (A2 — GlobalHttpExceptionFilter)

Introdotto `GlobalHttpExceptionFilter` `@Catch(HttpException)` registrato via `useGlobalFilters` in `main.ts`. Normalizza shape errore HTTP cross-endpoint:
...
```json
{
  ...
  "...": "extras preservati da DTO custom (es. timestamp TD-AJ)"
}
```

### Branch detection (4 path)

1. **DTO esplicito** `body.errorCode` (es. `AuthErrorResponse` `auth.service.ts:128`) → preserva.
2. **Legacy `body.code`** → mappa `code` → `errorCode` (transition path).
3. **Taxonomy wrapped in `body.message`** (pattern NestJS `new UnauthorizedException('E_FOO')`) → estrae stringa taxonomy → `errorCode`. **Discovery #49.**
4. **ValidationPipe 400** (array `body.message` da class-validator) → inietta `errorCode: 'E_VALIDATION'`.

Fallback: `STATUS_TO_DEFAULT_ERRORCODE` mapping (es. 401 → `E_UNAUTHORIZED`, 429 → `E_RATE_LIMITED`).

### Extras preservation (Discovery #48)

Custom field DTO (es. `timestamp` TD-AJ, futuri `retryAfter`) preservati via `extractExtras()` + spread `...extras` PRIMA dei field normalizzati nello `response.json()`: precedenza esplicita normalizzati safe by-design (campo `errorCode` in extras impossibile per via di NORMALIZED_KEYS set).

### LockoutExceptionFilter inheritance

`LockoutExceptionFilter` ora `extends GlobalHttpExceptionFilter` (non più `BaseExceptionFilter`). Motivo: APP_FILTER (DI priority) runna PRIMA di `useGlobalFilters`; senza inheritance, `super.catch()` invierebbe response bypassando normalizzazione globale. Inheritance garantisce single source of truth shape anche su lockout path. Side-effect Retry-After header preservato.

### Trade-off accettati

- **Logger.error solo 500+**: 4xx atteso, no noise. **TD-BH candidate** (security logging 401/403 ripetuti per fraud detection observability).
- **`isTaxonomyCode` regex permissive** `^E_[A-Z][A-Z0-9_]*$`: zero-touch per nuovi code, typo non rilevato. Mitigation parziale: `AuthErrorCode` enum (compile-time check in auth.service.ts). **TD-BG candidate** (restringi regex post migrazione call site).
- **Legacy call site invariati** (7+ `UnauthorizedException('E_*')` in `auth.service.ts:255,259,285,292,341,390,403` + `tenant.middleware.ts:49,64` + `tenants.controller.ts:48`): backward-compat via branch detection 3. **TD-BG follow-up** per convergenza stilistica.

### Files modificati

- `apps/api/src/common/filters/global-http-exception.filter.ts` (new, 184 LOC)
- `apps/api/src/main.ts` (+8, useGlobalFilters)
- `apps/api/src/auth/filters/lockout-exception.filter.ts` (refactor extends GlobalHttpExceptionFilter)
- `apps/api/src/auth/auth.service.ts` (+/-3, lockout body `code` → `errorCode`)
- `apps/api/src/auth/auth.module.ts` (+/-6, commento DI priority + TD-AY rationale)
- `apps/api/test/e2e/auth-login.e2e-spec.ts` (+/-12, assertion `code` → `errorCode` + bonus statusCode assert)

### Discovery refs

- **Discovery #48** — `extras` field preservation contract DTO custom (TD-AJ `timestamp` survives normalization). Rivelato da E2E test failure 1° run implementazione (filter v1 stripava timestamp).
- **Discovery #49** — NestJS `UnauthorizedException(code)` body semantica (taxonomy in `body.message`, NOT `body.errorCode` come ci si aspetterebbe). Detection branch necessario per backward-compat legacy call site. Rivelato da E2E test failure 1° run (`E_AUTH_TENANT_REQUIRED` finiva fallback `E_UNAUTHORIZED`).

## TD-BE Sub-2 resolution (sessione 15)

**Status:** RESOLVED 2026-05-19 via PR `chore/td-ay-td-be-sub2-cleanup` (cross-ref TD-BE ADR-0018).

### Decision (α1 — env override CI surgical)

`.github/workflows/ci.yml` job `e2e-playwright` env block: `THROTTLE_AUTH_LIMIT: '100'` (era `'5'`, default prod-safe). Heredoc `.env` riusa `${{ env.THROTTLE_AUTH_LIMIT }}` → auto-pickup nuovo valore.

**Scope surgical (NO over-apply):**

- Job `checks` (Vitest unit): mock `LockoutService`/`ThrottlerGuard`, no rate limit reale → bump non necessario.
- E2E Testcontainers backend (`pnpm test:e2e`): override interno via `apps/api/test/e2e/helpers/test-app.ts:56` (`process.env.THROTTLE_AUTH_LIMIT='100'`), indipendente da `.env` root.
- Dev locale Nicolò: `.env` root invariato (`THROTTLE_AUTH_LIMIT=5`, prod-safe). Override manuale `THROTTLE_AUTH_LIMIT=100 pnpm dev` se serve Playwright deterministic locale.

### Trade-off accettato

α1 (env override CI) preferito a γ (Redis reset selettivo `beforeEach`):

- **γ richiede** `globalSetup` Playwright + helper Redis test infra + eventuale endpoint admin debug per FLUSHDB selettivo → ~1h+ scaffolding, NON drop-in (verifica empirica STOP 0: NO helper esistente, webServer non spawn API).
- **α1 trivial** (1 line env block CI + commento rationale), riusa pattern esistente backend e2e (`test-app.ts:56`).

γ resta opzione future se test infra evolve (multi-shard Playwright, parallelizzazione richiede isolation per-worker).

### Discovery refs

- **Discovery #47** (sessione 14, ADR-0018) — full suite Playwright flake TD-H lockout cascading 429 quando `THROTTLE_AUTH_LIMIT=5` viene saturato da `auth.setup.ts` multi-tenant + auth-\* specs in 60s window.
