...
| **TD-AY** (scope espanso post-cleanup PR sessione 12) | Allineamento taxonomy `errorCode` unificata cross-endpoint **COMPLETA**. Include: (1) 401 endpoint mancanti (`/auth/refresh`, `/auth/logout`, `/auth/login-pin`, `/auth/pin-setup`) oggi shape NestJS default no `errorCode`; (2) 429 `LockoutExceptionFilter` (`code` → `errorCode` rename + body shape align — vedi [Discovery #42 ADR-0013](./ADR-0013-auth-e2e-hardening-b1.md#discovery-42--lockoutexceptionfilter-naming-inconsistency-code-vs-errorcode)); (3) 400 validation custom `ValidationPipe` exception factory (vedi [Discovery #43](#discovery-43--dto-class-validator-shape-non-coerente-con-taxonomy-td-aj)). Frontend `parseError()` resta drop-in compatibile (no refactor — già legge `body.errorCode` con fallback `E_UNKNOWN`). Trigger re-eval: F1+ aggiunge endpoint 400/429 con UX visible OR utente smoke browser segnala alert generico "Si è verificato un errore. Riprova" come blocker UX. | Bassa (UX legacy gap, no security) | ~45-60min realistic |
