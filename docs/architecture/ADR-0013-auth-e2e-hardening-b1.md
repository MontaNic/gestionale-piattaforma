...
| ----------------- | ----------------------------------------- | ----------------------------------------------------------------------------- |
| `/auth/login`     | `email:<email>`                           | TD-H: futuro `tenantId:email` post TD-2 ADR-0012 multi-tenant slug resolution |
...
| D4    | Lockout key per-tenant (`<tenantId>:<email>`)             | email-only                                                 | Rejected (per ora)        | TD-H carry-over post TD-2 ADR-0012 multi-tenant routing                                                                    |
...
## Tech Debt Accepted (14 voci nuove, B1)
...
| **TD-H** | Lockout   | `/auth/login` lockout key = `email:<email>` only (non per-tenant) → DoS-by-account-name accettato (un attacker che conosce un'email può bloccarla per 15min)                 | Post [TD-2 ADR-0012](./ADR-0012-frontend-auth-flow.md#td-2-multi-tenant-tenant-slug-resolution) multi-tenant slug resolution | ~30min |
...
- **DoS-by-account-name** (TD-H): chiunque conosca un'email può bloccarla per 15min. Mitigato dal duration limit. Risolvibile con captcha post-3-fail (out-of-scope F1) o lockout key per-tenant (TD-H trigger).
...
- LOC totali B1:
  - Production code: ~570 LOC (redis 86 + throttler module 80 + guards 102 + decorators 38 + utils 79 + lockout 152 + filter 58 + edits auth.service.ts ~100 + wiring ~30)
  - Test code: 303 LOC (lockout 197 + throttler helpers 106)
  - Docs: ADR-0013 (~28KB), PROGRESS update, README update
