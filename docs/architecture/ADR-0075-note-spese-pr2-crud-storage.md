...
- **Delete transazionale**: delete allegato = `row + StorageService.delete` nella **stessa tx applicativa** (`withTenantContextAtomicTx`); se lo storage throwa → rollback della delete DB. Delete nota (bozza) = cascade righe DB **+ cleanup esplicito dei file** storage nella stessa tx (il cascade non tocca lo storage). Test T12 (asserisce sul filesystem: `storage.get` risolve prima, rigetta dopo).

  **Trade-off consapevole (spec-locked, non svista).** Il filesystem non è transazionale: mettere `storage.delete` **dentro** la tx sceglie deliberatamente il rischio "riga risuscitata da un rollback → punta a un oggetto già cancellato" (record che referenzia un blob mancante) invece dell'alternativa "commit riga → poi delete file", che invertirebbe il rischio verso il **file orfano** (blob senza riga, **recuperabile** — è esattamente ciò che `TD-storage-gc` è previsto per spazzare). La spec ha lockato la variante in-tx (coerenza row↔blob al momento del commit felice); l'ordine alternativo resta l'opzione se in futuro si preferisce l'orfano-recuperabile al dangling-pointer. Referenza: `TD-storage-gc`.
